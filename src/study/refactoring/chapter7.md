...
- 우리 서비스에서 이용 중인 오픈소스 마켓 서버스는 서버와 클라이언트는 패키지를 모두 제공한다. 서버를 커스터마이징하면서 클라이언트도 커스터마이징할 필요가 생겼는데, 패키지를 그대로 사용하고 있어서 변경을 하지 않았다. 대신 직접 API를 호출해서 사용해야 했다. 비슷한 경우일까?

### 컬렉션 캡슐화하기는 프런트엔드에서 어떤 경우에 주로 발생할까?

- 사용을 많이 하는 배열이 있고, 그 배열 데이터를 prop으로 자식, 손자, 그 아래까지 drilling해서 사용한다고 가정해보자. 그리고 시간에 쫓기는 개발자 혹은 스파게티 코드에서 해당 배열을 다른 개발자가 사용한다면? 원본 배열을 변경하려는 시도를 할 확률이 높아진다. 다만 Vue에서는 다행히 prop을 직접 변경한다면 warning을 띄워준다.
