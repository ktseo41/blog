...
- 2.4 **오래 걸리는 리팩터링**의 범주 안에서 회사에서 진행하려고 하는 swiper 패키지 업데이트를 진행해볼 수도 있을 것 같다.

#### 관리자에게 리팩터링한다고 말하지 말았어야 했던 것 같다.

- 최근 레거시 코드와 빌드 환경을 업그레이드하기 위해서 리팩터링 기간을 요청한 적이 있는데 그러지 말았어야 했던 것 같다.
- 추후에는 평소 과정에서 리팩터링을 병행해봐야 할 것 같다.
